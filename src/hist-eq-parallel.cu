...
#define BLOCK_SIZE_HIST 256
#define BLOCK_SIZE_EQ 512
...
__global__ void compute_histogram(uint8_t *image, uint32_t *histogram, size_t width, size_t height, size_t color_channel, size_t cpp) {
    ...
    if (row < height && col < width) {
        atomicAdd(&histogram[image[index(col, row, color_channel, width, cpp)]], 1);
    }*/
    ...
    while (threadId < (width * height * cpp) / cpp) {
        atomicAdd(&histogram[image[threadId * cpp + color_channel]], 1);
        threadId += stride;
    }
}
